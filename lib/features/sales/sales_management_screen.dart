...
import 'package:bsoft_app_dist/core/constants/allowed_users.dart';
import 'package:bsoft_app_dist/features/sales/vente_pdf_screen.dart';
import 'package:bsoft_app_dist/features/sales/reglement_pdf_screen.dart';
import 'dart:io';

import 'package:bsoft_app_dist/features/sales/add_vente_screen.dart';
...
class _ClientDetailsScreenState extends State<ClientDetailsScreen> {
  ...
  Future<void> _showEditVenteDialog(Map<dynamic, dynamic> vente) async {
    ...
    selectedItems = dbItems.map((item) {
      ...
      return {
        ...
        'COD_ARTICLE': item['code_article'] ?? item['id_item']?.toString() ?? '',
        'DESIGNATION': item['item_name'] ?? item['designation'] ?? '',
        'price': unitPrice,
            ...
            return AlertDialog(
              ...
              title: Row(
                ...
                children: [
                  ...
                  IconButton(
                    icon: const Icon(Icons.edit, color: Color(0xFF19264C)),
                    onPressed: () async {
                      await _showEditItemsDialog(localSetState);
                    },
                    tooltip: "Modifier les articles",
                  ),
                ],
              ),
              content: Container(
  ...
  Future<void> _showEditItemsDialog(StateSetter parentSetState) async {
    ...
    // Initialize entered quantities and prices from selectedItems
    Map<String, int> enteredQuantities = {};
    Map<String, double> enteredPrices = {};
    
    for (var item in selectedItems) {
      String code = item['id_item']?.toString() ?? item['COD_ARTICLE']?.toString() ?? '';
      if (code.isNotEmpty) {
        if (item.containsKey('NBRE_COLIS') && item['NBRE_COLIS'] != null && (item['NBRE_COLIS'] as int) > 0) {
          enteredQuantities[code] = item['NBRE_COLIS'] as int;
          isCarton = true; 
        } else if (item.containsKey('QUANT_LIVRE') && item['QUANT_LIVRE'] != null && (item['QUANT_LIVRE'] as int) > 0) {
          enteredQuantities[code] = item['QUANT_LIVRE'] as int;
        }
        enteredPrices[code] = (item['price'] as num?)?.toDouble() ?? 0.0;
      }
    }
    ...
    await showDialog(
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, localSetState) {
            // Merge selectedItems and selectedDepotMarchandises to ensure all sale items are visible
            // Use a Map to keep unique items by their id_item or COD_ARTICLE
            Map<String, Map<String, dynamic>> allAvailableItemsMap = {};

            // First, add all depot items
            for (var item in selectedDepotMarchandises) {
              String key = item['id_item']?.toString() ?? item['COD_ARTICLE']?.toString() ?? '';
              if (key.isNotEmpty) allAvailableItemsMap[key] = item;
            }

            // Then, add selectedItems (in case they are missing from depot list)
            for (var item in selectedItems) {
              String key = item['id_item']?.toString() ?? item['COD_ARTICLE']?.toString() ?? '';
              if (key.isNotEmpty && !allAvailableItemsMap.containsKey(key)) {
                allAvailableItemsMap[key] = Map<String, dynamic>.from(item);
                // Ensure it has basic fields for display if it was just a skeleton from conversion
                allAvailableItemsMap[key]!['DESIGNATION'] ??= item['DESIGNATION'] ?? 'Produit Inconnu';
                allAvailableItemsMap[key]!['QUANT_LIVRE'] ??= 0; // Default available stock to 0 if not in depot
                allAvailableItemsMap[key]!['NBRE_COLIS'] ??= 0;
              }
            }

            List<Map<String, dynamic>> allAvailableItems = allAvailableItemsMap.values.toList();

            // Filter items based on search query
            List<Map<String, dynamic>> filteredItems = allAvailableItems.where((item) {
              String name = (item['DESIGNATION'] ?? item['item_name'] ?? '').toString().toLowerCase();
              return name.contains(searchQuery.toLowerCase());
            }).toList();

            // Sort filteredItems to show items with quantity > 0 at the top
            filteredItems.sort((a, b) {
              String keyA = a['id_item']?.toString() ?? a['COD_ARTICLE']?.toString() ?? '';
              String keyB = b['id_item']?.toString() ?? b['COD_ARTICLE']?.toString() ?? '';
              int qtyA = enteredQuantities[keyA] ?? 0;
              int qtyB = enteredQuantities[keyB] ?? 0;
              if (qtyA > 0 && qtyB == 0) return -1;
              if (qtyA == 0 && qtyB > 0) return 1;
              // If both > 0 or both 0, sort alphabetically
              return (a['DESIGNATION'] ?? '').toString().compareTo((b['DESIGNATION'] ?? '').toString());
            });

            double newTotal = 0.0;
            enteredQuantities.forEach((key, qty) {
              if (qty > 0) {
                // Try to find price/metadata in allAvailableItemsMap
                var article = allAvailableItemsMap[key];
                if (article != null) {
                  double currentPrice = enteredPrices[key] ?? 0.0;
                  if (currentPrice == 0) {
                    double basePrice = isGros
                        ? (article['PU_ART_GROS'] is num ? (article['PU_ART_GROS'] as num).toDouble() : (article['price'] as num?)?.toDouble() ?? 0.0)
                        : (article['PU_ART'] is num ? (article['PU_ART'] as num).toDouble() : (article['price'] as num?)?.toDouble() ?? 0.0);
                    if (isCarton) {
                      basePrice *= (article['COND'] is num ? (article['COND'] as num).toDouble() : 1.0);
                    }
                    currentPrice = basePrice;
                    enteredPrices[key] = basePrice;
                  }
                  double tvaVal = (article['TVA'] as num?)?.toDouble() ?? tva;
                  newTotal += qty * currentPrice * (1.0 + (tvaVal / 100.0));
                }
              }
            });
            ...
            return AlertDialog(
              ...
              title: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('Modifier les Articles', style: TextStyle(fontFamily: 'Bahnschrift')),
                  Row(
                    children: [
                      const Text('U', style: TextStyle(fontSize: 12)),
                      Switch(
                        value: isCarton,
                        onChanged: (value) {
                          localSetState(() => isCarton = value);
                        },
                      ),
                ...
                child: Column(
                  children: [
                    TextField(
                      ...
                      onChanged: (value) {
                        localSetState(() => searchQuery = value);
                      },
                    ),
                    const SizedBox(height: 10),
                    Expanded(
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          var article = filteredItems[index];
                          String key = article['id_item']?.toString() ?? article['COD_ARTICLE']?.toString() ?? '';
                          int qty = enteredQuantities[key] ?? 0;
                              ...
                              child: Row(
                                ...
                                children: [
                                  IconButton(
                                    ...
                                    onPressed: qty > 0 ? () {
                                      localSetState(() => enteredQuantities[key] = qty - 1);
                                    } : null,
                                  ),
                                  Expanded(
                                    child: InkWell(
                                      onTap: () async {
                                        int? resultQty = await showDialog<int>(
                                          context: context,
                                          builder: (context) {
                                            TextEditingController controller = TextEditingController(text: qty.toString());
                                            return AlertDialog(
                                        ...
                                        if (resultQty != null) {
                                          localSetState(() => enteredQuantities[key] = resultQty);
                                        }
                                      },
                                      child: Text(qty.toString(), textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                                    ),
                                  ),
                                  IconButton(
                    ...
                    const Divider(),
                    const SizedBox(height: 10),
                    Text('Total Articles: ${newTotal.toStringAsFixed(2)} DA', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.blue)),
                  ],
                ),
              ),
              actions: [
                ElevatedButton(
                  style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF141E46)),
                  onPressed: () {
                    ...
                    enteredQuantities.forEach((key, qty) {
                      if (qty > 0) {
                        try {
                          var article = allAvailableItemsMap[key];
                          if (article != null) {
                            double price = enteredPrices[key] ?? 0.0;
                            if (price == 0) {
                               price = isGros
                                  ? (article['PU_ART_GROS'] is num ? (article['PU_ART_GROS'] as num).toDouble() : (article['price'] as num?)?.toDouble() ?? 0.0)
                                  : (article['PU_ART'] is num ? (article['PU_ART'] as num).toDouble() : (article['price'] as num?)?.toDouble() ?? 0.0);
                              if (isCarton) {
                                price *= (article['COND'] is num ? (article['COND'] as num).toDouble() : 1.0);
                              }
                            }

                            Map<String, dynamic> newItem = {
                              'id_item': article['id_item'] ?? int.tryParse(article['COD_ARTICLE']?.toString() ?? ''),
                              'COD_ARTICLE': article['COD_ARTICLE'],
                              'DESIGNATION': article['DESIGNATION'],
                              'price': price,
                            };
                            if (isCarton) {
                              newItem['NBRE_COLIS'] = qty;
                            } else {
                              newItem['QUANT_LIVRE'] = qty;
                            }
                            updatedItems.add(newItem);
                          }
                        } catch (e) {
                          print("Error finding article during update: $e");
                        }
                      }
                    });
                  ...
                  child: const Text('Modifier', style: TextStyle(color: Colors.white, fontFamily: 'ZTGatha')),
                ),
                ElevatedButton(
                  style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFD9F4E9)),
                  onPressed: () => Navigator.pop(context),
                  child: const Text('Annuler', style: TextStyle(fontFamily: 'ZTGatha', color: Color(0xFF19264C))),
                ),
              ],
            );
          },
  ...
  Future<void> _updateVente(BuildContext dialogContext, String venteId) async {
    ...
    // Update CA: adjust by totalAmount difference
    double currentTotalCa = prefs.getDouble('total_chiffre_affaire') ?? 0.0;
    double caDiff = totalAmount - _oldTotalAmount;
    currentTotalCa += caDiff;
    await prefs.setDouble('total_chiffre_affaire', currentTotalCa);
    ...
    // Update CA in clientData: adjust by totalAmount difference
    double currentClientCa = 0.0;
    if (clientData['CA'] != null) {
      currentClientCa = double.tryParse(clientData['CA'].toString()) ?? 0.0;
    }
    double newClientCa = currentClientCa + caDiff;
    ...
    if (saleIdInt > 0) {
      await db.updateSale(saleIdInt, venteData, saleItems);
    }

    // Update client financials
    await db.updateClientFinancials(int.parse(widget.clientId), vers: newVers, ca: newClientCa);
                    ...
                    ElevatedButton(
                      ...
                      onPressed: canClickVente
                          ? () {
                              if (vanId == null) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text('Erreur: ID de la camionnette (Van ID) manquant. Veuillez vous reconnecter.')),
                                );
                                return;
                              }
                              Navigator.of(context).pop();
